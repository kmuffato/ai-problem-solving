...
{
    public class AStarStrategy<TState, TAction> : ISearchStrategy<TState, TAction>
        where TState : IEquatable<TState>
    {
        public delegate double Heuristic(TState source, TState destination);

        private readonly Heuristic h;
        ...
        {
            return node.PathCost + h(node.State, problem.GoalState);
        }
    }
}
